...
namespace Forum.Data
{
    public class DataMapper
    {
        ...
        private static Dictionary<string, string> LoadConfig(string configPath)
        {
            ...
            var config = contents.Select(x => x.Split('=')).ToDictionary(x => x[0], x => DATA_PATH + x[1]);
            return config;
        }
        ...
        public static List<Post> LoadPosts()
        {
            ...
            foreach (var line in dataLines)
            {
                ...
                var replys = args[5].Split(",", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();

                Post post = new Post(id, title, content, categoryId, authorId, replys);
                posts.Add(post);
            }
        ...
        public static void SavePosts(List<Post> posts)
        {
            ...
            {
                const string postFormat = "{0};{1};{2};{3};{4}";
                string line = string.Format(postFormat, post.Id, post.Title, post.CategoryId, post.AuthorId, string.Join(",", post.ReplyIds));
        ...
        public static List<Reply> LoadReplies()
        {
            ...
            foreach (var line in dataLines)
            {
                ...
                Reply reply = new Reply(id, contents, authorId, postId);
                replys.Add(reply);
            }
            return replys;
        }
